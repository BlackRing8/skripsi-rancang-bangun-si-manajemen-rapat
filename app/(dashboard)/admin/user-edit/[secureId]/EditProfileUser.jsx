...
export default function EditProfileUser({ secureId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/admin/list-user-manage/${secureId}/update-uj-user`, {
        method: "POST",
        body: JSON.stringify({
          secureId: secureId,
  ...
  const updateNama = async () => {
    try {
      const res = await fetch(`/api/admin/list-user-manage/${secureId}/update-name-user`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ secureId: secureId, name: namaBaru }),
      });
  ...
  const tambahNik = async () => {
    try {
      const res = await fetch(`/api/admin/list-user-manage/${secureId}/update-nik-user`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ secureId: secureId, nik: nikBaru }),
      });
  ...
  if (loading) {
    return <div className="m-6">Mengambil data...</div>;
  }
  return (
      ...
      {dataProfile && (
        <div className="flex space-y-2 flex-col xl:w-2/3 bg-white rounded-xl p-6 ">
          <label className="text-xl md:text-3xl">
            <b>Ubah data dari user id:</b> <label className="text-blue-500 underline">{secureId}</label>
          </label>
          ...
          <div className="flex items-center space-x-3">
            ...
            {dataProfile.nik ? (
              <button
                ...
                className="bg-blue-500 text-white px-5 py-2.5 rounded-lg disabled:hover:cursor-not-allowed"
              >
                Ubah data Nik{" "}
              </button>
            ) : (
